...
-type(amqp_method_field_name() ::
       ...
       | mandatory | global | exclusive | delivery_tag
       | routing_key | reply_text | queue | reply_code
       | exchange | passive | exclusive | delivery_tag
       | integer_result | identifier | capabilities | requeue
       | heartbeat | no_ack | consumer_tag | ticket
       | consumer_tag | staged_size | multiple | mechanisms
       | out_of_band | exchange | ticket | routing_key
       | class_id | arguments | nowait | consumer_tag
       | exchange | consumer_tag | integer_1 | queue
       | table | message_count | response | delivery_tag
       | queue | reply_code | exchange | method_id
       | durable | ticket | auto_delete | nowait
       | nowait | consumer_tag | auto_delete | identifier
       | ticket | arguments | nowait | exchange
       | active | reply_text | ticket | queue
       | redelivered | consumer_tag | reply_code | delivery_tag
       | insist | nowait | consumer_count | meta_data
       | reply_text | message_count | prefetch_size | routing_key
       | result | routing_key | message_count | queue
       | channel_max | routing_key | global | immediate
       | no_ack | integer_2 | exchange | string_result
       | no_ack | write | routing_key | delivery_tag
       | dtx_identifier | requeue | prefetch_size | queue
       | consumer_tag | routing_key | reply_code | routing_key
       | nowait | ticket | content_size | ticket
       | no_local | details | queue | queue
       | class_id | ticket | ticket | exchange
       | known_hosts | challenge | consumer_tag | exchange
       | frame_max | multiple | routing_key | type
       | known_hosts | mandatory | client_properties | consumer_tag
       | redelivered | nowait | internal | nowait
       | prefetch_count | arguments | consumer_tag | locales
       | active | exclusive | delivery_tag | string_2
       | nowait | virtual_host | cluster_id | content_checksum
       | prefetch_count | no_local | reply_text | heartbeat
       | prefetch_count | mandatory | passive | delivery_tag
       | if_unused | ticket | exchange | if_empty
       | exchange | version_major | reply_text | exchange
       | queue | queue | result | routing_key
       | ticket | passive | requeue | channel_max
       | identifier | arguments | exchange | string_1
       | host | routing_key | ticket | consume_rate
       | immediate | operation | consumer_tag | durable
       | locale | integer_op | reply_code | response
       | nowait | redelivered | version_minor | consumer_tag
       | integer_3 | server_properties | reply_code | global
       | immediate | ticket | nowait | realm
       | operation | delivery_tag | active | no_local
       | exclusive | exchange | queue | consumer_tag
       | string_op | exchange | exclusive | consumer_tag
       | ticket | frame_max | integer_4 | nowait
       | mechanism | ticket | message_count | consumer_tag
       | method_id | read )).
